...
export const OnOff =(props)=> {
    ...
    return(
        ...
        >
            ...
            <img style={{height:"18vh",maxWidth:"35%",cursor:"pointer",opacity:onoff==='1'?"1":"0.4"}} src={ICON[props.icon]}/>
        </div>
    );
}
...
export const ButtonBar =(props)=> {
    ...
    return(
        <div style={{display:"row"}}>
            <button onClick={()=> usePub(topic+"onoff", 1)}>
                <Curtain color={"#91b6f2"}/> 
                Открыть
            </button>
            <button onClick={()=> usePub(topic+"stop", 1)}>
                Стоп
            </button>
            <button onClick={()=> usePub(topic+"onoff", 0)}>
                <Curtain color={"#29292999"}/> 
                Закрыть
            </button>
        </div>
    );
